...
# ===== GLOBAL FILTER =====
def apply_global_filter(df, enable_global_filter, global_date_filter, date_col='datetime'):
...
def create_generic_pivot_table(df, metrics, metric_names, period_type, selectbox_key,
                                rate_cols=None, table_class="pivot-table",
                                period_options=None):
    """Tạo pivot table tổng quát cho các tab dùng cấu trúc Tuần/Tháng/Nội dung/Số liệu."""
    if period_options is None:
        period_options = ['Tuần', 'Tháng', 'Năm']

    st.markdown(f"### 📊 Bảng Pivot - Phân tích theo thời gian")
    ...
    if has_time_data:
        pivot_data = df_period.groupby(['period', 'period_sort'])[metrics].sum().reset_index()
        pivot_data = pivot_data.sort_values('period_sort', ascending=False)

        pivot_data = compute_change_columns(pivot_data, metrics, rate_cols)

        display_data = pivot_data.copy()
        display_columns = ['period']
        column_names = {'period': f'{period_type}'}

        for col in metrics:
            new_col = f'{col}_display'
            display_data[new_col] = display_data.apply(
                lambda row: format_cell_with_change(row, col, rate_cols), axis=1)
            display_columns.append(new_col)
            column_names[new_col] = metric_names.get(col, col)

        st.markdown(f"#### 📋 Tổng hợp theo {period_type} (bao gồm biến động)")
        df_display = display_data[display_columns].rename(columns=column_names)
        render_html_pivot_table(df_display, table_class)
    else:
        ...
        if 'Nội dung' in df.columns and 'Số liệu' in df.columns:
            ...
            summary_data['Số liệu'] = summary_data['Số liệu'].apply(clean_and_format_number)
