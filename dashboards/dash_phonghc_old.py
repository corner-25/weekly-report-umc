...
class PivotTableDashboard:
        ...
        try:
            ...
            if column_cols:
                pivot = pd.pivot_table(
                    ...
                    fill_value=0,
                    sort=False  # Giữ nguyên thứ tự từ data (đã sort giảm dần)
                )
            else:
            ...
            return pd.pivot_table(
                ...
                aggfunc='sum',
                fill_value=0,
                sort=False  # Giữ nguyên thứ tự từ data
            )    
    ...
    def load_data_from_dataframe(self, df):
        ...
        try:
            ...
            # DEBUG: Hiển thị các cột có trong file
            st.sidebar.info(f"🔍 DEBUG - Cột trong file: {list(self.data.columns)}")

            # Clean non-breaking spaces trước khi convert kiểu dữ liệu
            for col in self.data.select_dtypes(include=['object']).columns:
                self.data[col] = self.data[col].str.replace('\xa0', ' ', regex=False)
            ...
            # Xử lý cột Năm - QUAN TRỌNG: GIỮ NGUYÊN DỮ LIỆU TỪ FILE
            if 'Năm' not in self.data.columns:
                # Nếu file không có cột Năm, dùng năm được chọn từ selectbox
                default_year = st.session_state.get('default_year', datetime.now().year)
                self.data['Năm'] = default_year
                st.warning(f"⚠️ File không có cột 'Năm', đã gán năm {default_year} (từ selectbox)")
            else:
                # DEBUG: Hiển thị giá trị năm trước khi convert
                st.sidebar.info(f"🔍 DEBUG - Năm trong file (trước convert): {self.data['Năm'].unique()[:10]}")

                # Nếu có cột Năm, chuyển sang số và giữ nguyên giá trị từ file
                self.data['Năm'] = pd.to_numeric(self.data['Năm'], errors='coerce')

                # DEBUG: Hiển thị giá trị năm sau khi convert
                st.sidebar.info(f"🔍 DEBUG - Năm sau convert: {self.data['Năm'].unique()}")

                # Kiểm tra xem có giá trị null không
                if self.data['Năm'].isnull().any():
                    st.warning(f"⚠️ Có {self.data['Năm'].isnull().sum()} dòng thiếu năm, tự động gán năm {datetime.now().year}")
                    self.data['Năm'].fillna(datetime.now().year, inplace=True)
                # Đảm bảo Năm là số nguyên
                self.data['Năm'] = self.data['Năm'].astype(int)

                # DEBUG: Hiển thị giá trị năm cuối cùng
                st.sidebar.success(f"✅ DEBUG - Năm cuối cùng: {self.data['Năm'].unique()}")
            ...
            self.data['Tuần_Năm'] = self.data.apply(lambda x: f"W{int(x['Tuần'])}/{int(x['Năm'])}", axis=1)
            
            # KIỂM TRA TRÙNG LẶP
            key_cols = ['Danh mục', 'Nội dung', 'Năm', 'Tháng', 'Tuần']
            duplicates = self.data[self.data.duplicated(subset=key_cols, keep=False)]
            if not duplicates.empty:
                st.sidebar.error(f"⚠️ CẢNH BÁO: Có {len(duplicates)} dòng trùng lặp trong file!")
                st.sidebar.info("🔍 Các dòng trùng lặp sẽ được tự động gộp lại khi tạo pivot table")
    ...
    def merge_data(self, new_df):
        """Gộp dữ liệu mới vào dữ liệu hiện tại (multi-year support).
        Dữ liệu trùng key (Danh mục, Nội dung, Năm, Tháng, Tuần) sẽ được cập nhật."""
        if self.data is None:
            return self.load_data_from_dataframe(new_df)
        try:
            key_cols = ['Danh mục', 'Nội dung', 'Năm', 'Tháng', 'Tuần']
            # Xử lý new_df cơ bản
            new_df = new_df.copy()
            new_df.columns = new_df.columns.str.strip()
            if 'Năm' not in new_df.columns:
                default_year = st.session_state.get('default_year', datetime.now().year)
                new_df['Năm'] = default_year
            for col in ['Tuần', 'Tháng', 'Số liệu', 'Năm']:
                if col in new_df.columns:
                    new_df[col] = pd.to_numeric(new_df[col], errors='coerce')
            new_df['Năm'] = new_df['Năm'].astype(int)

            # Lấy data gốc (chỉ các cột cần thiết, bỏ cột tính toán)
            base_cols = [c for c in self.data.columns if c not in [
                'Quý', 'Tháng_Năm', 'Tuần_Tháng', 'Tuần_Năm',
                'Danh_mục_thứ_tự', 'Nội_dung_thứ_tự',
                'Tỷ_lệ_tuần_trước', 'Thay_đổi_tuần_trước'
            ]]
            existing = self.data[base_cols].copy()

            # Gộp: new_df ghi đè dòng trùng key
            merged = pd.concat([existing, new_df], ignore_index=True)
            merged = merged.drop_duplicates(subset=key_cols, keep='last')

            return self.load_data_from_dataframe(merged)
        except Exception as e:
    ...
    def _apply_priority_order(self):
        ...
        # Danh mục & Nội dung: tăng dần (theo priority)
        # Năm, Tháng, Tuần: GIẢM DẦN (tuần gần nhất hiển thị trước)
        ...
        # DEBUG: Hiển thị thứ tự sau khi sắp xếp
        sample_data = self.data[['Danh mục', 'Nội dung', 'Năm', 'Tháng', 'Tuần']].head(10)
        st.sidebar.info(f"🔍 DEBUG - Thứ tự sau sort (10 dòng đầu):\n{sample_data.to_string()}")
    ...
    def create_hierarchical_pivot_table_with_ratio(self, data, rows, cols, values, agg_func, show_ratio_inline):
        try:
            ...
            # ============= XỬ LÝ TRÙNG LẶP - QUAN TRỌNG =============
            # Kiểm tra và gộp các dòng trùng lặp (cùng Danh mục, Nội dung, Năm, Tuần)
            group_cols = []
            if rows:
                group_cols.extend(rows)
            if cols:
                group_cols.extend(cols)

            # Thêm các cột cần thiết cho việc nhóm
            if 'Danh_mục_thứ_tự' in data.columns and 'Danh_mục_thứ_tự' not in group_cols:
                group_cols.append('Danh_mục_thứ_tự')
            if 'Nội_dung_thứ_tự' in data.columns and 'Nội_dung_thứ_tự' not in group_cols:
                group_cols.append('Nội_dung_thứ_tự')

            # Loại bỏ trùng lặp bằng cách group by và sum
            if group_cols:
                # Kiểm tra số dòng trước và sau
                original_count = len(data)

                agg_dict = {'Số liệu': agg_func}

                # Giữ lại các cột biến động nếu có
                if 'Tỷ_lệ_tuần_trước' in data.columns:
                    agg_dict['Tỷ_lệ_tuần_trước'] = 'mean'  # Lấy trung bình tỷ lệ
                if 'Thay_đổi_tuần_trước' in data.columns:
                    agg_dict['Thay_đổi_tuần_trước'] = 'sum'  # Tổng thay đổi

                data = data.groupby(group_cols, as_index=False, dropna=False).agg(agg_dict)

                deduplicated_count = len(data)
                if original_count > deduplicated_count:
                    st.sidebar.warning(f"⚠️ Đã gộp {original_count - deduplicated_count} dòng trùng lặp")
            # =========================================================
            ...
            if 'Tuần' in cols and hasattr(pivot, 'columns'):
                ...
                if week_columns:
                    ...
                    if isinstance(current_columns[0], tuple):
                        # Với MultiIndex (ví dụ: (2026, 2) hoặc (2025, 52))
                        # Sắp xếp theo: Năm giảm dần, Tuần giảm dần
                        def sort_key(col_tuple):
                            ...
                            for element in col_tuple:
                                try:
                                    num = int(str(element))
                                    if num >= 2000 and num <= 3000:  # Năm
                                        years.append(num)
                                    elif 1 <= num <= 53:  # Tuần
                                        weeks.append(num)
                                except (ValueError, TypeError):
                                    continue
                            # Return tuple: (năm, tuần) để sort theo cả 2
                            year = years[0] if years else 0
                            week = weeks[0] if weeks else 0
                            return (year, week)

                        week_columns_sorted = sorted(week_columns, key=sort_key, reverse=True)
                    else:
                        # Với single index
                    ...
                    if isinstance(category_data, pd.DataFrame):
                        ...
                        for content in category_data.index:
                            ...
                            for col in category_data.columns:
                                ...
                                # Format số với dấu phẩy ngăn cách hàng ngàn
...
def main():
    ...
    # Thêm selectbox chọn năm (nếu file không có cột Năm)
    st.sidebar.subheader("📅 Chọn năm dữ liệu")
    selected_year = st.sidebar.selectbox(
        "Năm của dữ liệu (nếu file không có cột Năm)",
        options=[2026, 2025, 2024, 2023],
        index=1,  # Mặc định 2025
        help="Nếu file Excel không có cột 'Năm', hệ thống sẽ dùng năm này"
    )

    # Lưu vào session state để dùng trong load_data
    st.session_state['default_year'] = selected_year
    ...
    if data_source == "Upload file":
        uploaded_files = st.sidebar.file_uploader(
            ...
            help="Upload nhiều file để gộp dữ liệu nhiều năm (VD: file 2025 + file 2026)"
        )
        if uploaded_files:
            for uf in uploaded_files:
                try:
                    ...
                    if dashboard.data is not None:
                        # Gộp vào data hiện tại
                        if dashboard.merge_data(new_df):
                            st.sidebar.success(f"✅ Đã gộp: {uf.name}")
                            file_loaded = True
                        else:
                            st.sidebar.error(f"❌ Lỗi gộp: {uf.name}")
                    else:
                        # File đầu tiên — load bình thường
                        if dashboard.load_data_from_dataframe(new_df):
                            st.sidebar.success(f"✅ Đã tải: {uf.name}")
                            file_loaded = True
                        else:
                            st.sidebar.error(f"❌ Lỗi tải: {uf.name}")
                except Exception as e:
                    st.sidebar.error(f"❌ Lỗi đọc {uf.name}: {str(e)}")

            # Upload lên GitHub nếu kết nối OK
            if connected and file_loaded:
                if st.sidebar.button("☁️ Upload GitHub", use_container_width=True):
                    try:
                        success = manager.upload_new_file(dashboard.data, "merged_data.xlsx")
                        if success:
                            st.balloons()
                            time.sleep(1)
                            st.rerun()
            ...
            # Hiển thị tổng kết data đã gộp
            if file_loaded and dashboard.data is not None:
                years_in_data = sorted(dashboard.data['Năm'].unique())
                st.sidebar.info(f"📊 Dữ liệu: {len(dashboard.data):,} dòng | Năm: {', '.join(str(int(y)) for y in years_in_data)}")
    ...
    if file_loaded and dashboard.data is not None:
        # ═══════════════ FILTER NĂM NHANH (trên cùng) ═══════════════
        # Lưu bản gốc để filter không mất data
        if 'full_data_backup' not in st.session_state or st.session_state.get('data_version') != id(dashboard.data):
            st.session_state['full_data_backup'] = dashboard.data.copy()
            st.session_state['data_version'] = id(dashboard.data)

        full_data = st.session_state['full_data_backup']
        available_years = sorted(full_data['Năm'].unique())
        if len(available_years) > 1:
            year_options = ["Tất cả"] + [str(int(y)) for y in available_years]
            yr_cols = st.columns(len(year_options))
            selected_quick_year = st.session_state.get('quick_year_filter', 'Tất cả')
            for i, yr_opt in enumerate(year_options):
                with yr_cols[i]:
                    btn_type = "primary" if selected_quick_year == yr_opt else "secondary"
                    if st.button(f"📅 {yr_opt}", key=f"qyr_{yr_opt}", use_container_width=True, type=btn_type):
                        st.session_state['quick_year_filter'] = yr_opt
                        st.rerun()

            # Áp dụng filter năm nhanh (từ bản gốc)
            if selected_quick_year != 'Tất cả':
                dashboard.data = full_data[full_data['Năm'] == int(selected_quick_year)].copy()
            else:
                dashboard.data = full_data.copy()
            st.markdown("---")
        ...
        # Tabs cho các chế độ xem - Tạo tabs động dựa trên danh mục
        available_categories = list(aggregated_data['Danh mục'].unique()) if not aggregated_data.empty else []

        # Định nghĩa các tab chính và tab theo danh mục
        main_tabs = ["📋 Tất cả", "📊 Xu hướng", "💾 Xuất báo cáo"]
        category_tabs = []

        # Tạo tab cho từng danh mục có trong dữ liệu
        category_icons = {
            "Văn bản đến": "📥",
            "Văn bản phát hành": "📤",
            "Tổ xe": "🚗",
        ...
        # Sắp xếp danh mục theo thứ tự ưu tiên và tạo tab
        sorted_categories = sorted(available_categories, key=lambda x: dashboard.category_priority.get(x, 999))
        for cat in sorted_categories:
            icon = category_icons.get(cat, "📁")
            category_tabs.append(f"{icon} {cat}")

        # Tạo tab "Khác" cho các danh mục không phân loại
        # if any(cat not in category_icons for cat in available_categories):
        #     category_tabs.append("🔗 Khác")

        # CHỈ HIỂN THỊ 3 TABS CHÍNH (không hiển thị category tabs)
        all_tabs = main_tabs  # Bỏ category_tabs
        tabs = st.tabs(all_tabs)

        # TAB 1: TẤT CẢ (Pivot Table)
        with tabs[0]:
